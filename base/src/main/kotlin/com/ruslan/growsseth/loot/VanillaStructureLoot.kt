...
object VanillaStructureLoot {
    ...
    fun onModifyLootTables(id: Identifier, tableBuilder: LootTableModifier) {
        if (MiscConfig.modLootInVanillaStructures) {
            val poolBuilder = LootPool.lootPool()

            if (STRONGHOLD_LOOT.identifier() == id) {
            ...
            else if (RUINED_PORTAL_LOOT.identifier() == id) {
                poolBuilder
                    .add(LootItem.lootTableItem(GROWSSETH_BANNER_PATTERN))
            }
            else if (ANCIENT_CITY_LOOT.identifier() == id) {
                poolBuilder
                    .add(LootItem.lootTableItem(GROWSSETH_ARMOR_TRIM))
                SKULK_DISCS.forEach { poolBuilder.add(LootItem.lootTableItem(it)) }
            }
            else if (DUNGEON_LOOT.identifier() == id || MANSION_LOOT.identifier() == id) {
                getDiscs().forEach {
                    poolBuilder.add(LootItem.lootTableItem(it))
                }
            }
            else if (OCEAN_RUIN_COLD_LOOT.identifier() == id || OCEAN_RUIN_WARM_LOOT.identifier() == id) {
                poolBuilder
                    .add(LootItem.lootTableItem(GROWSSETH_POTTERY_SHERD))
            }
            else if (END_CITIES_LOOT.identifier() == id) {
                poolBuilder
