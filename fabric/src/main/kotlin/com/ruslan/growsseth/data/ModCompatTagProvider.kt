...
class ModCompatTagProviderItems(output: FabricDataOutput, val registries: CompletableFuture<HolderLookup.Provider>) :
    ...
    override fun addTags(arg: HolderLookup.Provider) {
        ...
        getOrCreateTagBuilder(GrowssethTags.COBBLEMON_BERRIES_EXCEPT_RARE)
    ...
    private fun FabricTagProvider<Item>.FabricTagBuilder.addOptional(item: ItemLike): FabricTagBuilder
        = this.addOptional(item.asItem().builtInRegistryHolder().key())

    private fun cobbleTag(id: String) =
        TagKey.create(Registries.ITEM, ResourceLocation.fromNamespaceAndPath("cobblemon", id))
